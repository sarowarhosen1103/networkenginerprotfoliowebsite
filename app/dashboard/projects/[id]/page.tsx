    ...
    const fetchProjectAndCategories = async () => {
      try {
        ...
        if (data.project) {
          ...
          setConfigurations(data.project.configurations || []);
        } else {
  ...
  if (loading) {
    return (
      <main className={`pt-24 pb-12 flex justify-center items-center min-h-screen ${isSidebarHidden ? "lg:ml-60" : "lg:ml-[360px]"}`}>
        <div className="font-label-mono text-primary-container animate-pulse">LOADING PROJECT DATA...</div>
      </main>
    );
  }
      ...
      <div className="p-gutter max-w-3xl mx-auto space-y-gutter w-full flex-grow flex flex-col">
        <div className="bg-surface-container/50 backdrop-blur-xl border border-outline-variant/30 p-8 flex flex-col">
          <div className="flex items-center gap-3 mb-8 border-b border-white/10 pb-4">
            ...
            <div>
              <h2 className="font-headline-md text-xl text-primary leading-none">MODIFY PROJECT</h2>
              <p className="font-label-mono text-xs text-outline mt-1 uppercase">Database Entry Update Form</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6 flex-grow font-label-mono">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-xs text-primary-container">PROJECT_TITLE *</label>
                <input
                  type="text"
                  required
            ...
            <div className="space-y-2">
              <MarkdownEditor
                label="PROJECT_RICH_DETAILS_AND_DOCUMENTATION"
                value={formData.content}
                onChange={(val) => setFormData({ ...formData, content: val })}
                placeholder="Enter rich details, step-by-step process, systems topology, or long documentation..."
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <label className="text-xs text-primary-container">IMAGE_URL (Optional)</label>
                <input
                  type="text"
              ...
              <div className="space-y-2">
                <label className="text-xs text-primary-container">PROJECT_URL / GITHUB_REPOSITORY (Optional)</label>
                <input
                  type="text"
              ...
              <div className="space-y-2">
                <label className="text-xs text-primary-container">DEMO_URL / LIVE_PREVIEW (Optional)</label>
                <input
                  type="text"
