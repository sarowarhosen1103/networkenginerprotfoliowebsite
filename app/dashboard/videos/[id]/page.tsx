...
export default function EditVideoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchVideoAndCategories = async () => {
      try {
        const catRes = await fetch("/api/categories?type=video");
        ...
        if (data.video) {
          setFormData({
            ...
            command: data.video.command || "",
            codeSnippet: data.video.codeSnippet || "",
            thumbnailUrl: data.video.thumbnailUrl || "",
            content: data.video.content || "",
          });
          setTags(data.video.tags || []);
          setConfigurations(data.video.configurations || []);
        } else {
  ...
  if (loading) {
    return (
      <main className={`pt-24 pb-12 flex justify-center items-center min-h-screen ${isSidebarHidden ? "lg:ml-60" : "lg:ml-[360px]"}`}>
        <div className="font-label-mono text-primary-container animate-pulse">LOADING VIDEO DATA...</div>
      </main>
    );
  }
      ...
      <div className="p-gutter max-w-3xl mx-auto space-y-gutter w-full flex-grow flex flex-col">
        <div className="bg-surface-container/50 backdrop-blur-xl border border-outline-variant/30 p-8 flex flex-col">
          <div className="flex items-center gap-3 mb-8 border-b border-white/10 pb-4">
            ...
            <div>
              <h2 className="font-headline-md text-xl text-primary leading-none">MODIFY VIDEO</h2>
